...
class IK:
    ...
    def displacement_and_axis(target, current):
        ...
        current - 4x4 numpy array repr esenting the "current" end effector orientation
    ...
    def distance_and_angle(G, H):
        ...
        D_h = H[:-1,-1]
        diff = D_g - D_h                 #doubt
        distance = np.linalg.norm(diff)    #doubt
        Rg = G[:-1,:-1]
        Rh = H[:-1,:-1]
        R_rel = np.linalg.inv(Rg) @ Rh
    ...
    def is_valid_solution(self,q,target):
        ...
        fk = FK()
        joint_positions, a_all, T0e = fk.forward(q)
        T0e = a_all[-1]
        print(f"T0e {T0e}")
        distance, angle = self.distance_and_angle(target,T0e)
        #check = (q <= self.lower) or (q >= self.upper)
        q_low = q > self.lower
        q_high = q < self.upper
        check = np.logical_and(q_low,q_high)
        if np.any(check==False):
            success = False
            message = "solution not found as q is not within the joint limits"
        elif (distance > self.linear_tol):
            success = False
            message = "solution not found becase distance > than linear tolerance"
        elif (angle > self.angular_tol):
            success = False
            message = "solution not found becase angle > than angle tolerance"
        else:
            success = True
    ...
    def end_effector_task(q,target,method):
        ...
        eof = np.hstack((disp, axis))
        if method == 'J_pseudo':
            J_pseudo = np.linalg.pinv (J, rcond = 0)
            dq = J_pseudo @ eof
        elif method == 'J_trans':
            dq = J.T @ eof
        ## END STUDENT CODE
    ...
    def inverse(self, target, seed, method, alpha):
        ...
        i = 0;
        while distance>=self.linear_tol or angle>=self.angular_tol :
            rollout.append(q) #list of list
            ...
            dq_null = np.dot(np.eye(7) - np.dot(J1, J), dq_center)
            dq = dq_ik + dq_null
...
if __name__ == "__main__":
    ...
    for i, q_pseudo in enumerate(rollout_pseudo):
        joints, pose, T0e = ik.fk.forward(q_pseudo)
        d, ang = IK.distance_and_angle(target, T0e)
    ...
    for i, q_trans in enumerate(rollout_trans):
        joints, pose, T0e = ik.fk.forward(q_trans)
        d, ang = IK.distance_and_angle(target,T0e)
